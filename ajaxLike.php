<?php 
    ...
    //スイーツのIDやユーザーIDは0から始まる可能性があるので issetを使って確認している。
    //スイーツの詳細画面にいるということはログインしていると思うが、ログインページへ飛ばないようにisLoginで確認し、ログインしていなかったとしてもheader関数をつかわない
    if(  isset($_POST['sweetsId']) && isset($_SESSION['user_id']) && isLogin() ){
        ...
        try{
            ...
            //DBへ接続
            $dbh = dbConnect();
            //まずはDBのお気に入りテーブルに自分が見ているスイーツがあるか確認している
            ...
            if(!empty($resultCount)){
                //デバッグ
                debug('このスイーツはお気に入りに存在しました。お気に入りからこのスイーツを削除します');
